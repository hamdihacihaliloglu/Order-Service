...
# original https://github.com/nginx/unit/blob/master/pkg/docker/docker-entrypoint.sh
...
if [ "$1" = "unitd" ]; then
    if /usr/bin/find "/var/lib/unit/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
        ...
        if /usr/bin/find "/docker-entrypoint.d/" -mindepth 1 -print 2>/dev/null | /bin/grep .; then
            ...
            while [ -S /var/run/control.unit.sock ]; do echo "$0: Waiting for control socket to be removed..."; /bin/sleep 1; done
...
# python /code/app/services/order_consumer.py &  # Consumer'ı arka planda çalıştır
